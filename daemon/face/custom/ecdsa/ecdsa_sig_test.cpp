...
std::vector<uint8_t> makeFixedTestBuffer()
{
    constexpr size_t SIGNATURE_SIZE = 240;
    //constexpr size_t ID_SIZE = 4;

    std::vector<uint8_t> buf;

    // ===== 1. 署名(240byte) 全て0 =====
    buf.resize(SIGNATURE_SIZE, 0x00);

    // ===== 2. IDの数 = 1 =====
    buf.push_back(1);

    // ===== 3. 経路情報 = 10.0.0.1 =====
    uint32_t ip = inet_addr("10.0.0.1"); // network byte order
    uint8_t ipBytes[ID_SIZE];
    std::memcpy(ipBytes, &ip, ID_SIZE);
    buf.insert(buf.end(), ipBytes, ipBytes + ID_SIZE);

    // ===== 4. 署名対象要素 = 777 =====
    // int (4byte) として格納
    uint32_t value = htonl(777);
    uint8_t valueBytes[4];
    std::memcpy(valueBytes, &value, 4);
    buf.insert(buf.end(), valueBytes, valueBytes + 4);
...
int main() {
    try {
        ...
        std::vector<uint8_t> testBuf = makeFixedTestBuffer();
        ...
        std::vector<uint8_t> signature1 = signer.OneHopsign(testBuf);
        
        std::cout << " -> Generated Signature Size: " << signature1.size() << " bytes" << std::endl;
        memcpy(testBuf.data(), signature1.data(), SIG_LENGTH);

        // シナリオ 2: 検証 (Verify)
        std::cout << "\n[Step 3] Verify (First Hop)..." << std::endl;
        
        // 検証実行
        bool result1 = signer.OneHopverify(testBuf);
